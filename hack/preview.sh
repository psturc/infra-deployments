...
[ -n "${JVM_BUILD_SERVICE_CACHE_IMAGE}" ] && yq -i e "(.spec.template.spec.containers[].env[] | select(.name ==\"JVM_BUILD_SERVICE_CACHE_IMAGE\").value) |= \"${JVM_BUILD_SERVICE_CACHE_IMAGE}\"" $ROOT/components/jvm-build-service/controller_patch.yaml
[ -n "${JVM_BUILD_SERVICE_REQPROCESSOR_IMAGE}" ] && yq -i e "(.spec.template.spec.containers[].env[] | select(.name ==\"JVM_BUILD_SERVICE_REQPROCESSOR_IMAGE\").value) |= \"${JVM_BUILD_SERVICE_REQPROCESSOR_IMAGE}\"" $ROOT/components/jvm-build-service/controller_patch.yaml
